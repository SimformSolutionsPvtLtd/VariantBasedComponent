...
function convertUrlToDeepLink(
  ...
  if (isDeepLinkType(routeName, params, DeepLink.MagicLink)) {
    return `variantbasedcomponent://auth/signIn/${encodeURIComponent(url)}/${params?.tenantId}/${
      params?.email
    }`;
  } else if (isDeepLinkType(routeName, params, DeepLink.ForgotPassword)) {
    return `variantbasedcomponent://auth/createNewPassword/${params?.oobCode}/${params?.tenantId}/${params?.email}`;
  }
  return undefined;
}
